...
import { Bell, CheckSquare, FolderKanban, KeyRound, Loader2, Palette, Settings, Shield, Users, Eye, EyeOff, ShieldCheck } from "lucide-react";
        ...
        <div className="flex items-center justify-between rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4">
          <div>
            ...
            <p className="mt-1 text-[11px] font-medium text-[var(--muted)]">
              {isEnabled
                ? "Your account is protected with TOTP QR Code scanning verification."
                : "Add an extra layer of security requiring a QR code scan from your Authenticator app."}
            </p>
          </div>
          ...
          <div>
            <button
              ...
              className={`rounded-xl px-3.5 py-2 text-xs font-black transition cursor-pointer shadow-sm ${
                isEnabled
                  ? "border border-rose-500/20 bg-rose-500/10 text-rose-500 hover:bg-rose-500/20"
                  : "bg-[var(--primary)] text-white hover:opacity-90"
              }`}
            >
              {isEnabled ? "Disable 2FA" : "Enable 2FA (QR Scan)"}
            </button>
          </div>
        </div>
